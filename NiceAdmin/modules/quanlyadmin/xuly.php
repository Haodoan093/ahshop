...
{
   $sql = "SELECT * FROM tbl_admin WHERE tensanpham='$tensanpham' OR masp='$masp'";
...
if (isset($_POST['dangky'])) { //them san pham 
   ...
   if (productExistsID($mysqli, $sodienthoai, $email, $idadmin)) {
    header('Location:../../users-profile.php?id=' . $idadmin . '&message=Thông tin đã tồn tại !');
      ...
      if ($hinhanh != '') {
         ...
         $sql_update = "UPDATE tbl_admin  SET hoten='" . $ten . "', thongtin='" . $thongtin . "',diachi='" . $diachi . "',sodienthoai='" . $sodienthoai . "',
